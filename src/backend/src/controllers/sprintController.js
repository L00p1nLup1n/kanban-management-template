    ...
    try {
        ...
        if (status && status !== currentStatus) {
            ...
            switch (status) {
                ...
                case 'completed':
                    // Auto-calculate completed points
                    ...
                    const totalCompletedPoints = completedTasks.reduce((sum, task) => {
                        return sum + (task.storyPoints || 0);
                    }, 0);

                    sprint.completedPoints = totalCompletedPoints;
                    break;
    ...
    try {
        ...
        return res.json({
            metrics: {
                ...
                pointCompletionRate:
                    totalPoints > 0
                        ? Math.round((completedPoints / totalPoints) * 100 * 10) / 10
                        : 0,
                capacity: sprint.capacity,
                capacityUtilization:
                    sprint.capacity > 0
                        ? Math.round((completedPoints / sprint.capacity) * 100 * 10) / 10
                        : 0,
...
async function calculateBurndown(sprint, tasks) {
    ...
    while (currentDate <= endDate) {
        ...
        const completedPoints = completedByThisDate.reduce(
